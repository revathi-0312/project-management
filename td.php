...
while ($row = $tasks_result->fetch_assoc()) {
    ...
    $existingStatusQuery = $conn->query("SELECT status FROM tasks WHERE task_id = {$row['task_id']}");
    ...
    if ($existingStatus != $row['status']) {
        ...
        // If task just became completed, notify the client
        if ($row['status'] === 'Completed') {
            // Get client_id from project
            $project_id = $row['project_id'];
            $project_result = $conn->query("SELECT client_id FROM projects WHERE project_id = $project_id");
            $client_row = $project_result->fetch_assoc();
            $client_id = $client_row['client_id'];

            // Insert notification
            $message = "Task '{$row['task_name']}' in project ID {$project_id} has been marked as Completed.";
            $stmt = $conn->prepare("INSERT INTO notifications (client_id, message) VALUES (?, ?)");
...
<body>
    <div class="container">
        ...
        <ul class="project-list">
            <!-- You can dynamically add projects here -->
        </ul>
